...
void readFile2D(const char* file, pcl::PointCloud<Point2D>::Ptr cloud, vector<string>& labels){
	string line;
	ifstream in (file);
	vector<string> tokens;
	if (in.is_open()){
...
void printCloud2D(pcl::PointCloud<Point2D>::Ptr cloud, vector<string>& labels, vector<vector<int> >& models){
    //fstream File;
    //File.open(file, ios::out);
    for (int i=0; i<cloud->size(); i++){
    	// 2D case
